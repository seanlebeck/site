...
var mapSVG = function(elem, options){
    ...
    this.methods = {
        ...
        panStart : function (e){
                ...
                if(!touchDevice)
                    jQuery.noconflict'body').on('mousemove', _this.panMove).on('mouseup', _this.panEnd);
        },
        panMove :  function (e){
                ...
                jQuery.noconflict'body').css({'cursor': 'move'});
        ...
        panEnd : function (e){
                ...
                jQuery.noconflict'body').css({'cursor': 'default'});
                ...
                if(!touchDevice)
                    jQuery.noconflict'body').off('mousemove', _this.panMove).off('mouseup', _this.panEnd);
        },
       ...
       renderSVGPath : function (item, parentTransform, parentStyle){
          ...
          var path = _data.R.path(jQuery.noconflictitem).attr('d'));          
       ...
       renderSVGImage: function (item, parentTransform, parentStyle){
          ...
          var src = jQuery.noconflictitem).attr('xlink:href'),
              x   = jQuery.noconflictitem).attr('x') || 0,
              y   = jQuery.noconflictitem).attr('y') || 0,
              w   = jQuery.noconflictitem).attr('width') || 0,
              h   = jQuery.noconflictitem).attr('height') || 0;
       ...
       renderSVGCircle: function (item, parentTransform, parentStyle){
          var x   = jQuery.noconflictitem).attr('cx') || 0,
              y   = jQuery.noconflictitem).attr('cy') || 0;
              r   = jQuery.noconflictitem).attr('r')  || 0;
       ...
       renderSVGEllipse: function (item, parentTransform, parentStyle){
          var x   = jQuery.noconflictitem).attr('cx') || 0,
              y   = jQuery.noconflictitem).attr('cy') || 0;
              rx  = jQuery.noconflictitem).attr('rx')  || 0;
              ry  = jQuery.noconflictitem).attr('ry')  || 0;
       ...
       renderSVGRect: function (item, parentTransform, parentStyle){
          var x   = jQuery.noconflictitem).attr('x') || 0,
              y   = jQuery.noconflictitem).attr('y') || 0,
              w   = jQuery.noconflictitem).attr('width') || 0,
              h   = jQuery.noconflictitem).attr('height') || 0;
              r   = jQuery.noconflictitem).attr('rx') || jQuery.noconflictitem).attr('ry') || 0;
       ...
       renderSVGText : function (textObj){
            ...
            var tspans = jQuery.noconflicttextObj).find('tspan');

            var x = parseFloat(jQuery.noconflicttextObj).attr('x')) || 0;
            var y = parseFloat(jQuery.noconflicttextObj).attr('y')) || 0;

            tspans.each(function(i,tspan){
                var t = _this.renderSVGTspan(jQuery.noconflicttspan), {x: x, y: y});
                
                t.attr(_this.styleSVG2Raphael(textObj));
                t.attr(_this.styleSVG2Raphael(jQuery.noconflicttspan)));
                
                t.transform( _this.transformSVG2Raphael(textObj) );
                t.transform( _this.transformSVG2Raphael(jQuery.noconflicttspan)));                
            });
       ...
       renderSVGTspan : function (textObj, parent){
            ...
            var x = parseFloat(jQuery.noconflicttextObj).attr('x')) || parent.x;
            var y = parseFloat(jQuery.noconflicttextObj).attr('y')) || parent.y;

            if(jQuery.noconflicttextObj).attr('dx'))
                x += parent.x + jQuery.noconflicttextObj).attr('dx');
            if(jQuery.noconflicttextObj).attr('dy'))
                y += parent.y + jQuery.noconflicttextObj).attr('dy');

            text = jQuery.noconflicttextObj).text();
            ...
            jQuery.noconflictrObj.node).css({
       ...
       initRaphaelObject : function (rObj, svgItem, parentTransform, parentStyle){
          ...
          rObj.id            = jQuery.noconflictsvgItem).attr('id') || rObj.type+(globalID++);
       ...
       styleSVG2Raphael : function(svgItem, parentStyle){
            ...
            var style       = {};
            var attrs       = jQuery.noconflictsvgItem).get(0).attributes;
            ...
            if(jQuery.noconflictsvgItem).attr('style')){
                var _style = jQuery.noconflictsvgItem).attr('style').split(';');
       ...
       transformSVG2Raphael : function(svgItem){
            ...
            var _transform = jQuery.noconflictsvgItem).attr('transform');
       ...
       getElementStyles : function (elem){
            if(!_browser.ie){
                ...
                if(jQuery.noconflictelem).attr('style')){
                    var def_styles = jQuery.noconflictelem).attr('style').split(';');
       ...
       regionAdd : function (_item){
            ...
            if(_data.ref[name].disabled){
                ...
                jQuery.noconflict_data.ref[name].node).css({'pointer-events' :'none'});
            }else{
            ...
            if(!_browser.ie && !_browser.firefox)
                jQuery.noconflict_data.ref[name].node).css({'vector-effect' : 'non-scaling-stroke'});
        ...
        markAddClickHandler : function(e){
            ...
            if(jQuery.noconflicte.target).is('image')) return false;
        ...
        setSize : function( width, height, responsive ){
            ...
            if(_data.options.responsive){
                ...
                jQuery.noconflictwindow).bind('resize.mapsvg', function(){
                ...
                jQuery.noconflictwindow).unbind('resize.mapsvg');
            }
        ...
        setTooltip : function (mode){
            ...
            _data.mapTip = jQuery.noconflict'<div class="map_tooltip"></div>');
            jQuery.noconflict"body").append(_data.mapTip);
            ...
            if(_data.options.tooltips.show == 'hover'){
                _data.$map.mousemove(function(e) {
                    _data.mapTip.css('left', e.clientX + jQuery.noconflictwindow).scrollLeft()).css('top', e.clientY + jQuery.noconflictwindow).scrollTop() + 30);
                });
            }
        ...
        setPopover : function (on){
            ...
            // Add tooltip container
            jQuery.noconflict"body").prepend('<div class="map_popover"><div class="map_popover_content"></div><div class="map_popover_close">x</div></div>');

            _data.mapPopover = jQuery.noconflict'.map_popover');
            ...
            _this.showPopover = function (e, content, pos){
                                    ...
                                    if(content){
                                        ...
                                        if(nx+_data.mapPopover.outerWidth(false) > jQuery.noconflictwindow).scrollLeft() + jQuery.noconflictwindow).width()) nx = (jQuery.noconflictwindow).scrollLeft() + jQuery.noconflictwindow).width()) - _data.mapPopover.outerWidth(false);
                                        if(ny+_data.mapPopover.outerHeight(false) > jQuery.noconflictwindow).scrollTop() + jQuery.noconflictwindow).height()) ny = (jQuery.noconflictwindow).scrollTop() + jQuery.noconflictwindow).height()) - _data.mapPopover.outerHeight(false);
                                        if(nx < jQuery.noconflictwindow).scrollLeft()) nx = jQuery.noconflictwindow).scrollLeft();
                                        if(ny < jQuery.noconflictwindow).scrollTop()) ny = jQuery.noconflictwindow).scrollTop();
        ...
        popoverOffHandler : function(e){
            if(!jQuery.noconflicte.target).closest('#map_popover').length)
                _this.hidePopover();
        },
        isNumber : function (n) {
        ...
        init    : function(opts, elem) {
            ...
            _data.map  = elem;
            _data.$map = jQuery.noconflictelem);
            _data.whRatio = 0;
            _data.isPanning = false;
            //_data.showTip = {};
            _data.markOptions = {};
            ...
            var loading = jQuery.noconflict'<div>'+_data.options.loadingText+'</div>').css({
                position: 'absolute',
                top:  '50%',
                left: '50%',
                'z-index': 1,
            ...
            loading.css({
                'margin-left': function () {
                    return -(jQuery.noconflictthis).outerWidth(false) / 2)+'px';
                },
                'margin-top': function () {
                    return -(jQuery.noconflictthis).outerHeight(false) / 2)+'px';
                }
            });
            ...
            $.ajax({
                ...
                success:  function(xmlData){
                    ...
                    if(_browser.ie && _browser.ie.old){
                        ...
                        if(_data.options.responsive)
                            jQuery.noconflictwindow).on('resize', _this.fluidResize);

                    }else{
                        _data.R = Raphael(_data.$map.attr('id'), '100%', '100%');

                        if(_data.options.responsive){
                            jQuery.noconflictwindow).on('resize', function(e){
                                _data.scale = _this.getScale();
                                _this.marksAdjustPosition();
                            });
                        }
                    ...
                    var parse = function(_item, parentTransform, parentStyle){
                        var type = jQuery.noconflict_item).get(0).tagName;
                    ...
                    var traverse = function (node, parentTransformString, parentStyle){
                        ...
                        var children = jQuery.noconflictnode).children();
                        
                        if (children.length){// && !jQuery.noconflictnode).is('text')) {
                            ...
                            jQuery.noconflictnode).children().each(function(i,n){
                              var _parentStyle = _this.styleSVG2Raphael(n, parentStyle);
                              traverse(jQuery.noconflictn), parentTransformString, _parentStyle);
                            });
                            
                        }// else{                
                            parse(jQuery.noconflictnode), parentTransformString, parentStyle);
                        //} 
  ...
  $.fn.mapSvg = function( opts ) {
    ...
    var id = jQuery.noconflictthis).attr('id');
