...
<div id="ad_reload">
<script>
var foo = "<?php echo $shuffledUrl; ?>";
// Updating the browser URL with Ajax loaded Ad
...
<div  id="GO" style="border-left: border-top-right: 1px solid #000;position:relative;right:55%;left:45%;z-index:333;top:-10px;cursor:pointer"class="circle3"><div  id="GO" class="inner3">shuffle</div></div>
<?php
...
include("showad_change.php");
echo '<div id="comment_counter">';
if(!$xadid){
	$xadid = $shufflethisad;
	}
	$sql = "SELECT a.*, ct.cityname as cityname, a.user_id, cat.catname, scat.subcatname, scat.catid, UNIX_TIMESTAMP(a.timestamp) AS timestamp, UNIX_TIMESTAMP(a.createdon) AS createdon, UNIX_TIMESTAMP(a.expireson) AS expireson, UNIX_TIMESTAMP(feat.featuredtill) AS featuredtill $xfieldsql
			FROM $t_ads a
...
$sender_username =  $_COOKIE[$ck_username];
$sender_userid = $_COOKIE[$ck_userid];
    ...
    $msg_sent2 = mysql_real_escape_string(htmlspecialchars($msg_sent, ENT_QUOTES));
   
	$adtitle = $ad['adtitle'];
	$adids = $ad['adid'];
	
	mysql_query("INSERT INTO `dojoep_messaging_db`.`conversations` SET conversation_subject = '$adtitle'");
	
	$conversation_id = mysql_insert_id();
	
		$sql2 = "INSERT INTO `dojoep_messaging_db`.`conversations_messages` 
		...
		$values[] = "({$conversation_id}, {$recipient_userid}, 0, 0)";
	
		$sql3 = "INSERT INTO `dojoep_messaging_db`.`conversations_members` (`conversation_id`, `user_id`, `conversation_last_view`, `conversation_deleted`)
                  VALUES " . implode(", ", $values);
                        
		mysql_query($sql3);

		
		$sql = "INSERT INTO `dojoep_messaging_db`.`vivaru_contact_form_save` 
...
<table class="postheader" width="1000px" cellpadding="0" cellspacing="0"> 
<tr>
...
<table class="postheader" width="1000px">
<tr>
...
if($show_cats_in_sidebar && !($xview == "main" || $xpostmode) && !$show_sidebar_always) { ?>
...
<?php
			...
			{ $html .= 'Please <a style="text-decoration:underline;" href="'.$script_url.'/login.html">Login</a> or <a style="text-decoration:underline;" href="'.$script_url.'/signup.html">Register</a> to contact this user.'; 
...
<div id="map" style="width: 250px; height: 110px;"></div>
...
//]]></script>
<?php } 
//echo '<div id="cccomment_counter"><span style="float:left" id="mesempty">'.$comment_count.'</span></div></div>';							
//echo $comment;
?>
</div>
 <script type="text/javascript">// <![CDATA[
	...
	$('.label').hide(); 
	 ...
	 $('.value').html('<img style=\"opacity:.7;padding-top:5px;min-height:14px;height:14px;width:auto;\" src="images/loadinggif.gif" />');
	
    $.ajaxSetup({ cache: false }); // This part addresses an IE bug. without it, IE will only load the first number and will never refresh   
//	$('#ad_reload').ajaxStart(function() { $(this).show(); });
	 $('#mydiv').show(); 
    $('#comment_counter').load('showad_change.php');
 
	$('#ad_reload').load('showad.php');
	 $('#mydiv').hide(); 
//	$('#ad_reload').ajaxComplete(function() { $(this).hide(); });
