<script type="text/javascript" src="/home/dojoep/public_html/_vti/jquery.js"></script>
...
///RSS retrieval
 require_once('/home/dojoep/public_html/_vti/magpierss/rss_fetch.inc');
	//$url = $_GET['url'];
	$url="http://boston.craigslist.org/cta/index.rss";
	$rss = fetch_rss( $url );
...
$uniqueid = str_replace(".html", "", $uniqueid);
echo "<br>";
...
$images = substring_between($output,'imgList = [','"];'); 
...
$varstrings = substring_between($output,"<h2 class","</h2>");
...
$content=substring_between($output,'<section id="postingbody">','<ul class="blurbs">');
//echo "content is: ".$content;
$creationdate=substring_between($output,'<date',$timezone);
...
VALUES
('".$price."','local','".$plink."','craigslist','".$title."','".$content."','".$city."','".$email."','2','','117.198.242.244.4ab4c865b71db','".$city."','".$category."','".$price."','',0,'','1',0,'1',NOW(),NOW()+INTERVAL 30 DAY,NOW());
";
	//echo $query;
